...
from utils.viz_utils import format_spines, AnnotateBars
    ...
    def evaluate_performance(self, X_train, y_train, X_test, y_test, cv=5, save=False, overwrite=True,
        ...
        for model_name, model_info in self.classifiers_info.items():
            ...
            if 'train_performance' in model_info.keys():
                ...
                df_performances.loc[len(df_performances)] = model_info['train_performance']
                # df_performances = df_performances.append(model_info['test_performance'])
                df_performances.loc[len(df_performances)] = model_info['test_performance']
                continue
            ...
            model_performance = pd.concat([train_performance, test_performance], axis=0)
            df_performances = pd.concat([df_performances, model_performance], axis=0)
